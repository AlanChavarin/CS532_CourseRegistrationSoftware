...
function StudentNotesSearchForm({ setStudentNotes }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_KEY}studentnotes/?${searchParams}`)
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col items-center justify-center gap-4 w-full max-w-md bg-white p-4 rounded-lg shadow-md border border-gray-300">
        <input 
          type="text" 
          placeholder="Search for student notes"
          name="searchTerm"
...
export default StudentNotesSearchForm